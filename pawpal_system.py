...
def _normalize_day_type(day_type: DayType | str) -> DayType:
	...
	raise ValueError("day_type must be DayType.WEEKDAY, DayType.WEEKEND, 'weekday', or 'weekend'")
...
class Owner:
	...
	def add_pet(self, pet: Pet) -> None:
		...
		if any(existing.pet_name.lower() == pet.pet_name.lower() for existing in self.pets):
			raise ValueError(f"A pet named '{pet.pet_name}' is already associated with {self.owner_name}")
		self.pets.append(pet)

	def remove_pet(self, pet: Pet) -> None:
		"""Remove a pet association from this owner."""
		if pet not in self.pets:
			raise ValueError(f"Pet '{pet.pet_name}' is not associated with {self.owner_name}")
...
class Scheduler:
	...
	def generate_owner_schedule(self, owner: Owner, day_type: DayType | str) -> Dict[str, List[Task]]:
		...
		# Gather essential tasks in deterministic order: owner pet order, then pet task order.
		...
		# Gather optional tasks across all pets and rank globally.
